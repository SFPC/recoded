...
//    }
//    
//    parameters.add(fontSize.set("Font Size", 70, 20, 400));
...
void CooperBauhaus::draw(){
  ...
  //  float t = sin(ofGetElapsedTimef()*speed);
    for (int i = 0; i < 6; i ++) {
        ofSetColor(colors[(int)floor(i/2.0)]);
        font.drawStringAsShapes(text, VISUALS_WIDTH/2 - r.width/2, VISUALS_HEIGHT - displacement*i - 10);
    }
    ofDisableBlendMode();
    ofPopStyle();
}
