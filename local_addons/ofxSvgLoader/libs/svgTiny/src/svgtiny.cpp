...
// added by Nick //
struct svgtiny_parse_state createStateForSvgNode( Poco::XML::Document* document, struct svgtiny_diagram* diagram ) {
    
    Poco::XML::Element* svg = document->documentElement();
	...
	state.diagram   = diagram;
	state.document  = document;
	state.viewport_width    = 500;
	state.viewport_height   = 500;
	svgtiny_parse_position_attributes( svg, state, &x, &y, &width, &height);
	diagram->width = width;
	diagram->height = height;
